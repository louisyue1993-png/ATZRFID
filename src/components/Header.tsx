...
export default function Header() {
  ...
  const currentLocale = (getLocaleFromPathname(pathname || '/') ?? defaultLocale) as Locale;
  const targetLocale: Locale = currentLocale === 'en' ? 'zh' : 'en';

  const toLocalizedPath = (href: string) => withLocalePath(href, currentLocale);

  const toggleLanguage = () => {
    router.push(withLocalePath(pathname || '/', targetLocale));
    setMobileMenuOpen(false);
  };

  const t = getLocaleTexts(currentLocale).nav;
    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/80 backdrop-blur-xl supports-[backdrop-filter]:bg-white/60">
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          {/* Logo */}
          <Link href={toLocalizedPath('/')} className="flex items-center group">
            <img
              src="/logo.svg"
              alt="ATZ RFID Logo"
          ...
          <div className="flex items-center space-x-2">
            ...
            {/* Cart */}
            <Link href={toLocalizedPath('/cart')}>
        ...
        <nav className="md:hidden border-t border-slate-200 bg-white/95 backdrop-blur-xl">
          <div className="container mx-auto px-4 py-4 space-y-1">
            ...
            >
              ...
              <span className="text-sm font-semibold text-slate-900">{currentLocale === 'en' ? '简体中文' : 'English'}</span>
            </Button>
          </div>
        </nav>
      )}
